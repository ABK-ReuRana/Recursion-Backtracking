...
public class l008_Backtracking2 {
    
    // crossWord from pepcoding Level2 :-
    ...
    // es code se Arrangement aayegi sets ki
    public static int kSubsetSum_Arr(int arr[], int[] setSum, ArrayList<ArrayList<Integer>> sets, int idx, int k){
        ...
        int count=0;
        // k options for each element
        for(int i=0;i<k;i++){
             setSum[i]+=arr[idx];
             sets.get(i).add(arr[idx]);

             count+= kSubsetSum_Arr(arr, setSum, sets, idx+1, k);

             sets.get(i).remove(sets.size()-1);
             setSum[i]-=arr[idx];
        }
    ...
    public static int kSubSetSum(int[] arr, int idx, int[] setSum, ArrayList<ArrayList<Integer>> sets) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            // agr leader bn gya toh aage calls prevent krenge
